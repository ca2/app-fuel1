...
namespace lootfilter
{
   ...
   render::filter::filter(string strPath) :
      filter()
   {
   ...
   bool render::initialize_render(string strId)
   {
      ...
      if (Application.m_strAppId == "app-core/flag")
      {
         ...
         m_filtera.add(filter("matter://cat.gif"));
         m_filtera.add(filter("matter://nanosvg/23.svg"));
         m_filtera.add(filter("matter://main/rock_with_a_mask.png"));
         m_filtera.add(filter("matter://picachu_by_rondex.png"));
         m_filtera.add(filter("matter://totoro_plus_plus.png"));
      ...
      for (auto & filter : m_filtera)
      {

         m_pview->data_get("filter." + filter.m_strPath, filter.m_bNew);
   ...
   void render::lootfilter_render()
   {

      {
         ...
         lootfilter_render(pdib->get_graphics());
   ...
   void render::lootfilter_render(::draw2d::graphics * pgraphics)
   {
      ...
      //   if(m_bLite)
      {
         ...
         lootfilter_render_lite_view(pgraphics);

      }
      //   else
      //   {
      //
      //      lootfilter_render_full_view(pdib->get_graphics());
      //
      //   }
   ...
   void render::lootfilter_render_lite_view(::draw2d::graphics * pgraphics)
   {
...
//      else
      ...
      string strLootFilter;

      {

         synch_lock slText(m_pmutexText);

         strLootFilter = get_lootfilter().c_str(); // rationale : string allocation fork *for multithreading*

      }

      pgraphics->set_font(m_font);

      size = pgraphics->GetTextExtent(strLootFilter);
         ...
         else
         {
...

            {
               ...
               if (m_dib.initialize(m_cxCache1, m_cyCache1, int (m_dMaxRadius)))
               {
                  ...
                  m_dib->get_graphics()->text_out((m_cxCache1 - size.cx) / 2, (m_cyCache1 - size.cy) / 2, strLootFilter);
...

      {
         ...
         if (strLootFilter != get_lootfilter() || m_cxCache1 != m_cxTarget || m_cyCache1 != m_cyTarget || m_dibTemplate->area() <= 0)
            return;
...
//      else if (m_bAlternate)
      ...
      //if(!m_bAlternate)
      {
         ...
         pgraphics->text_out((m_cx - size.cx) / 2, (m_cy - size.cy) / 2, strLootFilter);
      ...
      if(!m_bFirstDone)
      {
         ...
         if (strLootFilter == get_lootfilter() && m_cxCache1 == m_cxTarget && m_cyCache1 == m_cyTarget)
         {
   ...
   void render::lootfilter_render_full_view(::draw2d::graphics * pgraphics)
   {
...
//
      ...
      string strLootFilter = get_lootfilter();

      ::size size = pgraphics->GetTextExtent(strLootFilter);
...

      {
         ...
         if (!Session.savings().is_trying_to_save(::aura::resource_display_bandwidth))
         {
            ...
            m_dib->get_graphics()->text_out((m_cx - size.cx) / 2, (m_cy - size.cy) / 2, strLootFilter);
      ...
      if (strLootFilter == get_lootfilter() && m_cx == m_rectClient.width() && m_cy == m_rectClient.height())
      {
      ...
      if (Application.m_etype == application::type_mili)
      {

         {
...

            {
               ...
               strLootFilter = m_pview->get_processed_lootfilter().c_str();
            ...
            if (m_bNewLayout)
            {
               ...
               class size size = pgraphics->GetTextExtent(strLootFilter);
      ...
      if (m_bFast || !m_bFirstDone || ::get_tick_count() - m_dwLastFast < m_dwFastAnime)
      {
         ...
         if (m_bFast || m_dibFast.is_null())
         {
...

            {
               ...
               lootfilter_fast_render(m_pview->get_processed_lootfilter());
   ...
   void render::defer_update_filter()
   {
      ...
      for (auto & filter : m_filtera)
      {
         ...
         if (filter.m_bNew && !filter.m_b)
         {

            m_stra23.add_unique(filter.m_strPath);

            filter.m_b = true;

            m_pview->data_set("filter." + filter.m_strPath, filter.m_b);

         }
         else if (!filter.m_bNew && filter.m_b)
         {

            m_stra23.remove(filter.m_strPath);
   ...
   void render::lootfilter_fast_render(const string & strLootFilter)
   {
      ...
      path->add_string((m_cx - size.cx) / 2, (m_cy - size.cy) / 2, strLootFilter, m_font);
