...
namespace lootfilter
{
   ...
   void view::_001OnLayout(::message::message * pobj)
   {
...

      {
         ...
         if(m_strNewLootFilter.is_empty())
         {
   ...
   string view::get_processed_lootfilter()
   {

      synch_lock slText(&m_mutexText);

      string str = get_lootfilter();
...
//      int c = pcre2_get_ovector_count(m_pmd);
   ...
   string view::get_lootfilter()
   {

      synch_lock sl(&m_mutexText);

      if(m_strLootFilter != m_strNewLootFilter)
      {

         m_strLootFilter = m_strNewLootFilter;

      }

      if(m_strLootFilter.is_empty())
      {
         ...
         if(m_prender->m_bAlternate)
         {
            ...
            if(m_dFps != 0.0)
            {
               return "Rolling " + Application.m_strAlternateLootFilter;
            }
            else
            {
               return Application.m_strAlternateLootFilter;
            }
         ...
         else
         {
            ...
            if(m_dFps != 0.0)
            {
               return "Rolling " + Application.m_strLootFilter;
            }
            else
            {
               return Application.m_strLootFilter;
            }
   ...
   void view::defer_check_on_draw_layout()
   {
      ...
      if (m_prender != NULL)
      {
         ...
         if (get_processed_lootfilter() != m_prender->m_strLootFilter)
         {

            m_prender->m_strLootFilter = get_processed_lootfilter().c_str(); // rationale : string allocation fork *for multithreading*
   ...
   void view::set_lootfilter(string strText)
   {

      m_strNewLootFilter = strText;
